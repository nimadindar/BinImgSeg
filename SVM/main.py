...
def train_svm_classifier(X, y, C=2.0, gamma="scale"):
    ...
    svc = SVC(
        ...
        probability=True, class_weight=class_weight, random_state=0, verbose=True, shrinking=False
    )
...
def predict_image_svm(clf, img_rgb_u8, scrib_u8, size=384, refine=True, thr=0.5):
    ...
    H, W, D = F.shape
    X = F.reshape(-1, D)
    probs = clf.predict_proba(X)[:, 1].reshape(H, W).astype(np.float32)
...
def main():
    ...
    ap.add_argument("--out", type=str, default="SVM/outputs_svm")
