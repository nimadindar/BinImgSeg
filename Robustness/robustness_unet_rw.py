...
from Unet_RW.utils import imread, save_mask, crf_refine  
...
def predict_unet_prob(model, device, img_u8, size=384):
    ...
    pr = torch.sigmoid(model(x))[0, 0].detach().cpu().numpy()  # size x size
...
def evaluate_robustness(args):
    ...
    if args.perturb == "gaussian":
        ...
        csv_name = f"robustness_gaussian_st_{int(args.std) if float(args.std).is_integer() else args.std}.csv"
    elif args.perturb == "rotation":
        perturb = make_rotation_aug(args.deg)
        perturb_desc = f"rotation_deg{args.deg}"
        csv_name = f"robustness_rotation_dg_{int(args.deg) if float(args.deg).is_integer() else args.deg}.csv"
    else:
    ...
    rows = []
    fg_iou_list, bg_iou_list, miou_list = [], [], []
    fg_drop_list, bg_drop_list, miou_drop_list = [], [], []
    ...
    for p in tqdm(img_paths, desc=f"Robustness: {perturb_desc}"):
        ...
        fg_iou_list.append(fg_iou); bg_iou_list.append(bg_iou); miou_list.append(miou)
        fg_drop_list.append(fg_drop); bg_drop_list.append(bg_drop); miou_drop_list.append(miou_drop)


    drops = np.array(miou_drop_list, dtype=np.float32)
    top2_idx = np.argsort(-drops)[:2] if len(drops) >= 2 else np.arange(len(drops))
    top2_names = [rows[i][0] for i in top2_idx]


    mean_fg_iou   = float(np.mean(fg_iou_list))   if fg_iou_list   else 0.0
    mean_bg_iou   = float(np.mean(bg_iou_list))   if bg_iou_list   else 0.0
    mean_miou     = float(np.mean(miou_list))     if miou_list     else 0.0
    mean_fg_drop  = float(np.mean(fg_drop_list))  if fg_drop_list  else 0.0
    mean_bg_drop  = float(np.mean(bg_drop_list))  if bg_drop_list  else 0.0
    mean_miou_drop= float(np.mean(miou_drop_list))if miou_drop_list else 0.0


    csv_path = os.path.join(args.save_csv_dir, csv_name)
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["image", "perturb", "params",
                    "fg_iou", "bg_iou", "miou",
        ...
        w.writerow([])
        w.writerow(["MEANS"])
        w.writerow(["mean_fg_iou", f"{mean_fg_iou:.6f}"])
        w.writerow(["mean_bg_iou", f"{mean_bg_iou:.6f}"])
        w.writerow(["mean_miou",   f"{mean_miou:.6f}"])
        w.writerow(["mean_fg_change(1-iou)", f"{mean_fg_drop:.6f}"])
        w.writerow(["mean_bg_change(1-iou)", f"{mean_bg_drop:.6f}"])
        w.writerow(["mean_miou_change(1-iou)", f"{mean_miou_drop:.6f}"])
    ...
    print(f"Means — FG IoU: {mean_fg_iou:.4f}, BG IoU: {mean_bg_iou:.4f}, mIoU: {mean_miou:.4f}")
    print(f"Mean changes — FG: {mean_fg_drop:.4f}, BG: {mean_bg_drop:.4f}, mIoU: {mean_miou_drop:.4f}")
...
def main():
    ...
    ap.add_argument("--clean_dir", type=str, default=f"Unet_RW/{folder}/preds")
    ap.add_argument("--save_csv_dir", type=str, default=f"Robustness/results_{folder}")
